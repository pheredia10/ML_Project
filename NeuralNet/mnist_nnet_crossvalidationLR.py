...
########## SOMETHING IS WRONG. TESTING ACCURACIES ARE ABOVE 100%  #########################
...
from kcross import kfoldcrossNodes, kfoldcrossRates
import sys
sys.path.append("..")
import utils
from utils import *
...
# Cross Validation
LRvec=np.array([.01 ,.05, .1])
validationAccuracy_array=np.zeros(shape=(3,3))
j=0
for i in [100,1000,10000]:
    X,Y=X_train_tot[0:i,:],y_train_tot[0:i]
    validationAccuracy_array[:,j]=kfoldcrossRates(X,Y,LRvec)
    j=j+1
bestLRInd=np.argmin(validationAccuracy_array,axis=0)
bestLRs=LRvec[bestLRInd]
...
for i in [100 ,1000, 10000]:
    ...
    model.compile(loss='categorical_crossentropy', optimizer=SGD(lr=LR, momentum=.1), metrics=["accuracy"])
    ...
    counter+=1
print('Validation Accuracy Table: (Rows=Learning Rates, Columns=Sample Sizes)',validationAccuracy_array)
